    ...
    format!(
        "You are a professional technical translator specializing in Unix/Linux system manual pages (man pages).\n\
         ...
         3. ABSOLUTELY PRESERVE the exact formatting, syntax, macros (e.g., .TH, .SH, .SS, .TP, .B, .I, .BR, .IR, .PP, etc.), and structure of the original man page source (which uses troff/groff formatting). The output must be valid man page source code. Do NOT convert the document to Markdown, HTML, or any other format.
          IMPORTANT: Add `.na` (no fill, no adjust) immediately after the `.TH` header line and after every `.P`/`.PP`/`.TP`/`.IP` paragraph macro. This is critical because groff's default filling and justification will insert excessive spaces into translated text (especially for CJK languages) and break lines at awkward positions. Also, break long text lines at natural CJK break points (after 。，、；：etc.) so each line is at most ~76 characters wide.
